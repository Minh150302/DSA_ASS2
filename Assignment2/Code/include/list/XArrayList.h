...
bool XArrayList<T>::removeItem(T item, void (*removeItemData)(T))
{
	...
	int cursor = 0;
	while (cursor != count){
		found = XArrayList<T>::equals(data[cursor], item, this->itemEqual);
		if(found){
			if(removeItemData)
				removeItemData(data[cursor]);
...
int XArrayList<T>::indexOf(T item)
{
	...
	for(int i = 0; i < count; i++){
		if(XArrayList<T>::equals(data[i], item, this->itemEqual))
			return i;
	}
...
void XArrayList<T>::checkIndex(int index)
{
	...
	if(index > count || index < 0)
		throw std::out_of_range("inappropriate index");
